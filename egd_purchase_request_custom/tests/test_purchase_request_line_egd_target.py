...
class TestPurchaseRequestLineEgdTarget(EGDSaleBlanketOrderCommon):
    ...
    def _prepare_blanket_product_cost(self, blanket, product, price_unit, quantity):
        ...
        return self.env["egd.sale.blanket.order.product"].create(
            {
    ...
    def _prepare_blanket_service_cost(self, blanket, product, price_unit, quantity):
        self.env["egd.sale.blanket.order.service"].search(
            [("blanket_order_id", "=", blanket.id), ("product_id", "=", product.id)]
        ).unlink()
        return self.env["egd.sale.blanket.order.service"].create(
            {
                "blanket_order_id": blanket.id,
                "product_id": product.id,
                "quantity": quantity,
    ...
    def test_target_value_product_branch_above_true(self):
        ...
        line._compute_egd_estimated_unit_cost()
        line._compute_egd_target()

        self.assertEqual(line.egd_target_value, price_unit)
        self.assertEqual(line.egd_target_quantity, quantity)
    ...
    def test_target_value_without_analytic_distribution_returns(self):
        ...
        self.assertEqual(line.egd_target_value, 0.0)
        self.assertEqual(line.egd_target_quantity, 0.0)
        self.assertTrue(line.egd_target_above)
