...
from .common import EGDSaleBlanketOrderCommon


class TestEgdSaleBlanketOrderCustom(EGDSaleBlanketOrderCommon, TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_00_full_workflow(self):
        ...
        """
        rec_product = self.env["egd.sale.blanket.order.product"].create(
            {
        ...
        rec_service = self.env["egd.sale.blanket.order.service"].create(
            {
                "blanket_order_id": self.blanket.id,
        ...
        self.blanket._compute_egd_total_product_costs()
        self.blanket._compute_egd_total_service_costs()
        self.blanket._compute_egd_total_costs()
        self.assertEqual(
            self.blanket.egd_total_product_costs,
            sum(self.blanket.egd_order_product_ids.mapped("amount_total")),
        )
        self.assertEqual(
            self.blanket.egd_total_service_costs,
            sum(self.blanket.egd_order_service_ids.mapped("amount_total")),
        )
        self.assertEqual(
            self.blanket.egd_total_costs,
            self.blanket.egd_total_product_costs + self.blanket.egd_total_service_costs,
        )
        ...
        self.blanket._compute_egd_account_analytic_line_ids()
        self.assertEqual(self.blanket.egd_account_analytic_line_count, 2)
        self.assertEqual(len(self.blanket.egd_account_analytic_line_ids), 2)
        action_analytic = self.blanket.action_show_egd_account_analytic_line()
        ...
        self.blanket._compute_egd_mis_cash_flow_forecast_line_ids()
        self.assertEqual(
            self.blanket.egd_mis_cash_flow_forecast_line_count,
            len(self.blanket.egd_mis_cash_flow_forecast_line_ids),
        )
        action_forecast = self.blanket.action_show_egd_mis_forecast()
        ...
        self.assertEqual(len(self.blanket.egd_sale_order_plan_ids), 2)

        self.blanket.state = "draft"
        self.blanket.egd_sale_order_plan_ids._compute_to_order()
        for p in self.blanket.egd_sale_order_plan_ids:
            self.assertFalse(p.to_order)
        self.blanket.egd_sale_order_plan_ids._compute_ordered()
        for p in self.blanket.egd_sale_order_plan_ids:
            self.assertFalse(p.ordered)

        self.blanket._compute_egd_ip_sale_order_plan()
        self.assertFalse(self.blanket.egd_ip_sale_order_plan)

        self.blanket.write({"egd_use_sale_order_plan": True})
        ...
        self.blanket._compute_egd_ip_sale_order_plan()
        self.assertTrue(self.blanket.egd_ip_sale_order_plan)

        plan = self.blanket.egd_sale_order_plan_ids[0]
        ...
        self.blanket.egd_sale_order_plan_ids._compute_to_order()
        plans = self.blanket.egd_sale_order_plan_ids.sorted("installment")
        first = plans[0]
        second = plans[1]
        ...
        self.assertEqual(len(self.blanket.egd_sale_order_plan_ids), 3)
        last_plan_num = max(self.blanket.egd_sale_order_plan_ids.mapped("installment"))
        for plan in self.blanket.egd_sale_order_plan_ids:
            self.assertEqual(plan.last, plan.installment == last_plan_num)

        self.assertTrue(self.blanket.remove_order_plan())
        self.assertFalse(self.blanket.egd_sale_order_plan_ids)

        self.blanket.create_order_plan(2, date.today(), 1, "month")
        plan = self.blanket.egd_sale_order_plan_ids[0]
        dup = self.env["egd.sale.blanket.order.sale.order.plan"].create(
            {
